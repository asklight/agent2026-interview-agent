...
public class ProjectProfileAnalysisValidator {
    ...
    private static final Pattern ASCII_FACT_TOKEN = Pattern.compile("[A-Za-z][A-Za-z0-9+#.\\-]*");
    ...
    public ProjectProfileAnalysis validate(ProjectProfileAnalysis analysis, String sourceDescription) {
        return validate(analysis, sourceDescription, null, null);
    }

    public ProjectProfileAnalysis validateEvidenceBacked(ProjectProfileAnalysis analysis,
                                                          String sourceDescription,
                                                          List<ProjectFactEvidence> responsibilityEvidence,
                                                          List<ProjectFactEvidence> architectureEvidence) {
        if (responsibilityEvidence == null || architectureEvidence == null) {
            invalid("职责和架构的原文依据不能为空");
        }
        return validate(analysis, sourceDescription, responsibilityEvidence, architectureEvidence);
    }

    private ProjectProfileAnalysis validate(ProjectProfileAnalysis analysis,
                                             String sourceDescription,
                                             List<ProjectFactEvidence> responsibilityEvidence,
                                             List<ProjectFactEvidence> architectureEvidence) {
        if (analysis == null) {
        ...
        for (String responsibility : responsibilities) {
            if (responsibilityEvidence == null && !containsNormalized(sourceDescription, responsibility)) {
                invalid("职责“" + responsibility + "”无法在项目原文中找到依据");
            }
        }
        for (String architectureFact : architecture) {
            if (architectureEvidence == null && !containsNormalized(sourceDescription, architectureFact)) {
                invalid("架构事实“" + architectureFact + "”无法在项目原文中找到依据");
            }
        }
        if (responsibilityEvidence != null) {
            validateFactEvidence(responsibilities, responsibilityEvidence, sourceDescription, "职责");
        }
        if (architectureEvidence != null) {
            validateFactEvidence(architecture, architectureEvidence, sourceDescription, "架构事实");
        }
    ...
    private void validateFactEvidence(List<String> summaries,
                                      List<ProjectFactEvidence> evidence,
                                      String sourceDescription,
                                      String field) {
        if (summaries.size() != evidence.size()) {
            invalid(field + "与原文依据数量不一致");
        }
        for (int index = 0; index < summaries.size(); index++) {
            ProjectFactEvidence item = evidence.get(index);
    ...
    private void requireAsciiFactTokens(String fact, String sourceDescription, String field) {
        String normalizedSource = sourceDescription.toLowerCase(Locale.ROOT);
        Matcher matcher = ASCII_FACT_TOKEN.matcher(fact);
        while (matcher.find()) {
            String token = matcher.group().toLowerCase(Locale.ROOT);
            if (!normalizedSource.contains(token)) {
                invalid(field + "包含原文不存在的英文技术或术语“" + matcher.group() + "”");
            }
        }
    }

    private void requireLexicalAnchor(String summary, String sourceFragment, String field) {
        if (containsNormalized(sourceFragment, summary)) {
            return;
        }
        Set<String> summaryBigrams = hanBigrams(summary);
        Set<String> fragmentBigrams = hanBigrams(sourceFragment);
        long matched = summaryBigrams.stream().filter(fragmentBigrams::contains).count();
        double coverage = summaryBigrams.isEmpty() ? 0 : (double) matched / summaryBigrams.size();
        if (matched < 2 || coverage < 0.25d) {
            invalid(field + "概括与 sourceFragment 缺少足够词汇关联");
        }
    }

    private Set<String> hanBigrams(String value) {
        StringBuilder characters = new StringBuilder();
        Matcher matcher = HAN_CHARACTER.matcher(value == null ? "" : value);
        while (matcher.find()) {
            characters.append(matcher.group());
        }
        Set<String> bigrams = new LinkedHashSet<>();
        for (int index = 0; index + 1 < characters.length(); index++) {
            bigrams.add(characters.substring(index, index + 2));
        }
        return bigrams;
    }
